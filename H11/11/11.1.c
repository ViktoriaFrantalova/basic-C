...
//vypise informace o crambinu do souboru a pomoci g2 knihovny nakresli kruhove schema residui
...
int j = 0;
    ...
    for(i = 0; i < residue_count; i++)
  {
    ...
    {
	if(strcmp(items[i].residue_name, residue_types[j].code3) == 0)
	{
	  items[i].residue_type = j;
	}
    }
  }
 return 0;
...
void kruh()
{
  ...
  for(i = 0; i <= residue_count; i++)
    {
      g2_pen(dev, g2_ink(dev, residue_types[items[i].residue_type].color_r, residue_types[items[i].residue_type].color_g,
