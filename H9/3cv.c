...
{ //vypise informacie o residuach
  ...
  for (n = 0; n < pocet_atomov; n++)
  {
    ...
    {
      resid[pocet_resid].posledny_atom = n - 1; //index posledneho atomu
      pocet_resid++;
...
void printPepBack() // funkcia, ktorá vyhľada...
{
  for (int i = 0; i < pocet_resid; i++) // ...pre kazde residium...
  {
    for (int j = resid[i].prvy_atom; j < resid[i].posledny_atom; j++) // ....pre všetky jeho atomy...
    {
      if (strcmp(" N  ", atoms[j].menoatomu) == 0) // ..N po ktorom štrukturne nasleduje alfa uhlík CA, uhlik C a kyslík O
      {
        printf("Residium: %4d  %-3.3s, peptidicka pater proteinu %d, %d, %d, %d\n", resid[i].cisloresidua, resid[i].menoresidua, j + 1, j + 2, j + 3, j + 4); // za N = (j+1) vyhladam CA(j+2), C.. a O..
      }
    }
  }
}
