...
//program, vypisujuc informacie o crambine do noveho subora

typedef struct
{                       // definicia struktury
  char nazovzaznamu[7]; // record_name
  int cisloatomu;       // atom_number
  char menoatomu[5];    // atom_name
  char alt_loc;         // alternativna pozicia
  char chain_id;        // identifikator proteinoveho retazca 21
  char i_code;          // kod indikujuci vlozenie residua 26
  float hodnota;        // occupancy
  float tepelny_faktor; // temp_factor
  char menoresidua[4];  // residue_name
  int cisloresidua;     // residue_number
  float x, y, z;
  char symbolprvku[3];  // element_name
  char formal_naboj[3]; // fomalny_charge
} ATOM;
...
{ // otvorenie vstupneho suboru
  ...
  while (feof(f) == 0)
  {
    ...
    if (strncmp(buf, "ATOM", 4) == 0 || strncmp(buf, "HETATM", 6) == 0)
    {
      ...
      strncpy(atoms[pocet].menoatomu, buf + 12, 4); // nacitanie mena atomu
